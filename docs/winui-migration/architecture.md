...
SimpleFile today is a single Tauri 2 process: a Svelte 5 WebView talks to a Rust backend through `invoke`, `listen`, and one `Channel` (`list_directory`). `src-tauri/src/lib.rs` registers 74 `#[tauri::command]` handlers, three plugins (`dialog`, `opener`, `updater`), SQLite `DbState`, and shared `AppState`. Settings and workspace state live in WebView `localStorage`; tags live in `{app_data_dir}/metadata.db`; smart folders live in `{app_data_dir}/smart_folders.json`.

The migration keeps that Rust domain as a sibling IPC service and replaces only the renderer and Tauri host glue with a native C# WinUI 3 UI process. The chosen transport is **Windows named pipes + JSON-RPC 2.0** (length-prefixed frames). One UI process owns exactly one service process, connected on a per-session / per-PID pipe so two SimpleFile instances never collide. Svelte/Tauri remain buildable and shippable until an explicit retirement PR after parity gates pass.
...
- Panic hook writes `%LOCALAPPDATA%\SimpleFile\startup.log` (`src-tauri/src/main.rs`).
...
- Keep `{app_data_dir}/metadata.db`, `{app_data_dir}/smart_folders.json`, and `%LOCALAPPDATA%\SimpleFile\startup.log` on their current paths.
...
```mermaid
flowchart LR
  subgraph uiProc["UI process — SimpleFile.exe"]
    XAML["WinUI 3 shell"]
...
| --- | --- | --- |
| Parent / UI | `SimpleFile.exe` | WinUI 3 window, workflows, named-pipe client, job object owner |
...
1. **Primary:** after `ConnectNamedPipe`, the service calls `GetNamedPipeClientProcessId` and accepts only if it equals `--parent-pid`. Optionally also verify that pid’s image is `SimpleFile.exe` (dev: the `dotnet` / App host). Mismatch → close the pipe and keep listening until the parent dies.
...
```mermaid
sequenceDiagram
  participant UI as SimpleFile.exe
...
| Mode | Service path |
| --- | --- |
| Release / portable / installed | Same directory as `SimpleFile.exe` |
...
| `simplefile-service` | bin `simplefile-service` | No | Named-pipe server, JSON-RPC dispatcher, `IpcHost` impl, panic log to `%LOCALAPPDATA%\SimpleFile\startup.log` |
...
**Gate 3 is not done** until steps E–I have removed `Channel`, `AppHandle`, `tauri::State`, and plugin traits from domain modules, and PR 10’s contract tests exercise all 74 methods against core. Do not skip to the service while those modules are still `mod` (private) inside `src-tauri`.
...
New solution, no project under `frontend/`. Target: unpackaged WinUI 3 on **Windows 10 2004 (build 19041)+ / Windows 11 x64**. TFM is **`net8.0-windows10.0.19041.0`** with `SupportedOSPlatformVersion` `10.0.19041.0`. Do not advertise 1809; the TFM will not run there. Do not bump to .NET 10 in this migration; that would be a later, separate TFM change.
...
| --- | --- | --- | --- |
| `SimpleFile.App` | `net8.0-windows10.0.19041.0` | Core, Ipc, Windows App SDK | Window (1200×800, min 800×600, centered, title `SimpleFile - File Explorer`), XAML shells, `FolderPicker`, drag-drop, process/job lifetime, Activate |
| `SimpleFile.Ipc` | `net8.0-windows` | none of WinUI | Named-pipe client, framing, 74 methods, event multicast, error mapping |
| `SimpleFile.Core` | `net8.0-windows` | Ipc | Port of `frontend/src/lib/app/*`, `appState.ts`, transfer/search/navigation workflows |
| `SimpleFile.Tests` | `net8.0-windows` | Core, Ipc | **Required.** Serialization parity, client framing, workflow unit tests. `dotnet test` is part of `check:winui`. |
...
3. **Service `main` (before accept):** install the same panic logger as `src-tauri/src/main.rs` (`[%timestamp%] SimpleFile panic` + location + message) to `%LOCALAPPDATA%\SimpleFile\startup.log`; construct `AppState`; call `init_db` via `Host::app_data_dir()` (same schema + seed as `db.rs`).
...
sequenceDiagram
  ...
  Setup->>Setup: Relaunch SimpleFile.exe
```
...
- NSIS (passive) is responsible for replacing files and relaunching `SimpleFile.exe`. Packaging must configure `nsProcess` / close-list for `SimpleFile.exe` and `simplefile-service.exe` (and `simplefile.exe` while dual-stack Tauri still exists). If a future packager is proven **not** to relaunch, a follow-up may let the UI start the new exe **after** the installer process exits with success — that is not the default.
...
| `%LOCALAPPDATA%\SimpleFile\startup.log` | Panic hook (`main.rs`) — **not** under the identifier folder |

`Host::app_data_dir()` in the service **must** resolve `%APPDATA%\com.simplefile.desktop`, not `%LOCALAPPDATA%\SimpleFile` and not a WinUI `ApplicationData` folder. Inventing a new folder orphans tags and smart folders for every 1.1.0 user.
...
GitHub Release (dual-stack)
  ...
  ├── SimpleFile_*_x64-winui-setup.exe        # NEW WinUI NSIS
  ├── SimpleFile_*_x64-winui.msi              # NEW WinUI MSI (not deferred)
...
Smoke scripts today look for `simplefile.exe` under `src-tauri/target/**/release`, not inside the zip. Keep that distinction. When WinUI becomes the default artifact name `SimpleFile_*_x64-portable.zip`, the **inner** images are `SimpleFile.exe` and `simplefile-service.exe` (the runtime names, not a versioned rename). That matches the NSIS close-list and the job-object image check.
...
```json
{
  ...
  "platforms": {
    "windows-x86_64": {
      "signature": "<minisign>",
      "url": "https://github.com/conniecombs/SimpleFile-Windows/releases/download/v1.2.0/SimpleFile_1.2.0_x64-winui-setup.nsis.zip"
    }
  }
}
```
...
Portable zip after WinUI default: zip filename stays `SimpleFile_*_x64-portable.zip`; inner files are `SimpleFile.exe` + `simplefile-service.exe` (+ WASDK self-contained deps). Update smoke scripts that look for `simplefile.exe` under `target\` in the same packaging PR.
...
| 3 | IPC service + contract tests | Tauri | PRs 1–10: core extract finished (no Tauri types in domain modules), `simplefile-service` implements 74 methods + 5 events + `list_directory.chunk`. Updater commands return the unconfigured stub string. Golden JSON + parity script green. `src-tauri` still ships. Svelte still the UI. |
...
Do not log path contents of user files at info level. Panic log stays append-only in `%LOCALAPPDATA%\SimpleFile\startup.log`.
...
| Signal | Where | What |
| --- | --- | --- |
| Panic | `%LOCALAPPDATA%\SimpleFile\startup.log` | Same format as `main.rs` (`[epoch] SimpleFile panic`, location, message). Both UI-adjacent service and Tauri bin. |
...
| 3 | .NET TFM | **Keep `net8.0-windows10.0.19041.0`.** Windows 10 2004+. No .NET 10 bump in this migration. |
...
- `src-tauri/src/main.rs` — `%LOCALAPPDATA%\SimpleFile\startup.log`
...
| 20 | `feat: WinUI NSIS/MSI/portable packaging` | `scripts/build-release.ps1`, smoke scripts, workflows | PRs 18–19 | Side-by-side artifacts including `SimpleFile_*_x64-winui.msi`. Tauri still default. |
...
5. **C# split:** `SimpleFile.App` (WinUI), `SimpleFile.Ipc` (client), `SimpleFile.Core` (workflows), **required** `SimpleFile.Tests`. Unpackaged WinUI 3, WASDK self-contained, TFM **`net8.0-windows10.0.19041.0`**, min OS Windows 10 2004. Keep multi-instance (no mutex). Hand-sync DTOs + parity script (no typeshare). Cargo package version is canonical; MSBuild reads it.
6. **Methods are the 74 existing command names.** Top-level params stay camelCase (`TauriCommandMap`); nested structs and results stay snake_case (`models.rs` / `types.ts`). `FileEntry.itemCount` is not on the wire.
...
14. **`install_update` does not call `app.restart()` in the service.** After verify + start NSIS it returns and emits `app.restart-required`. **The old UI exits first** (job kills the service). NSIS replaces files and relaunches `SimpleFile.exe`. The old UI does not `Process.Start` the new exe.
