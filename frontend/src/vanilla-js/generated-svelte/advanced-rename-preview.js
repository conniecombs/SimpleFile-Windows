...
//#endregion
//#region src/lib/components/advanced-rename-preview/AdvancedRenamePreview.svelte
var root = /* @__PURE__ */ from_html(`<div class="adv-preview-state"> </div>`);
var root_1 = /* @__PURE__ */ from_html(`<div class="adv-preview-state adv-preview-error"> </div>`);
var root_2 = /* @__PURE__ */ from_html(`<div><div class="adv-preview-names"><span class="adv-preview-old"> </span> <span class="adv-preview-arrow">-&gt;</span> <span class="adv-preview-new"> </span></div> <div class="adv-preview-path"> </div></div>`);
var root_3 = /* @__PURE__ */ from_html(`<!> <!>`, 1);
function AdvancedRenamePreview($$anchor, $$props) {
	push($$props, true);
	let extraCount = prop($$props, "extraCount", 3, 0), limit = prop($$props, "limit", 3, 500), message = prop($$props, "message", 3, ""), mode = prop($$props, "mode", 3, "empty"), rows = prop($$props, "rows", 19, () => []), totalRows = prop($$props, "totalRows", 3, 0);
	...
	var consequent = ($$anchor) => {
		...
		reset(div);
		template_effect(() => set_text(text, message() || "Building preview..."));
		append($$anchor, div);
	};
	...
	var consequent_2 = ($$anchor) => {
		...
		reset(div_2);
		template_effect(() => set_text(text_2, message() || "No matching files."));
		append($$anchor, div_2);
	};
	var alternate = ($$anchor) => {
		var fragment_1 = root_3();
		var node_1 = first_child(fragment_1);
		each(node_1, 19, rows, (row, index) => `${row.oldName}-${row.newName}-${row.detail}-${index}`, ($$anchor, row) => {
			var div_3 = root_2();
			...
			template_effect(($0) => {
				set_class(div_3, 1, $0);
				set_text(text_3, get(row).oldName);
				set_text(text_4, get(row).newName);
				set_text(text_5, get(row).error || get(row).detail);
			}, [() => clsx(rowClass(get(row)))]);
			append($$anchor, div_3);
		});
		...
		var consequent_3 = ($$anchor) => {
			...
			reset(div_6);
			template_effect(() => set_text(text_6, `Showing first ${limit() ?? ""} of ${totalRows() ?? ""} targets.`));
			append($$anchor, div_6);
		};
		if_block(node_2, ($$render) => {
			if (extraCount() > 0) $$render(consequent_3);
		});
		append($$anchor, fragment_1);
	};
	if_block(node, ($$render) => {
		if (mode() === "loading") $$render(consequent);
		else if (mode() === "error") $$render(consequent_1, 1);
		else if (mode() === "empty") $$render(consequent_2, 2);
...
//#endregion
//#region src/lib/components/advanced-rename-preview.ts
var mountedPreviews = /* @__PURE__ */ new WeakMap();
function clearAdvancedRenamePreview(target) {
	if (!target) return;
...
function renderAdvancedRenamePreview(target, props) {
	if (!target) return;
...
//#endregion
export { clearAdvancedRenamePreview, renderAdvancedRenamePreview };
