...
//#endregion
//#region src/lib/components/breadcrumb/BreadcrumbTrail.svelte
var root = /* @__PURE__ */ from_html(`<span class="breadcrumb-separator" aria-hidden="true">/</span>`);
var root_1 = /* @__PURE__ */ from_html(`<button type="button"> </button> <!>`, 1);
function BreadcrumbTrail($$anchor, $$props) {
	push($$props, true);
	let segments = prop($$props, "segments", 19, () => []);
	const BREADCRUMB_FOCUS_EVENT = "simplefile:breadcrumb-focus";
	const BREADCRUMB_NAVIGATE_EVENT = "simplefile:breadcrumb-navigate";
	function emitBreadcrumbEvent(type, event, segment, index, detail = {}) {
	...
	function handleSegmentClick(event, segment, index) {
		emitBreadcrumbEvent(BREADCRUMB_NAVIGATE_EVENT, event, segment, index);
	}
	function handleSegmentKeydown(event, segment, index) {
		switch (event.key) {
			case "ArrowLeft":
			...
			case "End":
				event.preventDefault();
				emitBreadcrumbEvent(BREADCRUMB_FOCUS_EVENT, event, segment, index, { targetIndex: segments().length - 1 });
				break;
		}
	}
	var fragment = comment();
	each(first_child(fragment), 19, segments, (segment, index) => `${segment.path}-${index}`, ($$anchor, segment, index) => {
		...
		if_block(node_1, ($$render) => {
			if (!get(segment).current) $$render(consequent);
		});
		template_effect(() => {
			set_class(button, 1, `breadcrumb-segment${get(segment).current ? " current" : ""}`);
			set_attribute(button, "data-path", get(segment).path);
			set_attribute(button, "aria-current", get(segment).current ? "page" : "false");
			set_text(text, get(segment).label);
		});
		delegated("click", button, (event) => handleSegmentClick(event, get(segment), get(index)));
		delegated("keydown", button, (event) => handleSegmentKeydown(event, get(segment), get(index)));
...
//#endregion
//#region src/lib/components/breadcrumb.ts
var mountedBreadcrumbs = /* @__PURE__ */ new WeakMap();
function clearBreadcrumbTrail(target) {
	if (!target) return;
...
function renderBreadcrumbTrail(target, props) {
	if (!target) return;
...
//#endregion
export { clearBreadcrumbTrail, renderBreadcrumbTrail };
