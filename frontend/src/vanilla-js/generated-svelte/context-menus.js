...
//#endregion
//#region src/lib/components/context-menus/ColumnHeaderMenu.svelte
var root$1 = /* @__PURE__ */ from_html(`<label class="column-menu-item"><input type="checkbox"/> <span> </span></label>`);
var root_1$1 = /* @__PURE__ */ from_html(`<div class="column-header-menu-title">Columns</div> <label class="column-menu-item column-menu-item--disabled"><input type="checkbox" checked="" disabled=""/> <span>Name</span></label> <!> <button type="button" class="column-menu-button" data-column-reset-widths="">Reset column widths</button>`, 1);
function ColumnHeaderMenu($$anchor, $$props) {
	let columns = prop($$props, "columns", 19, () => []);
	var fragment = root_1$1();
	each(sibling(first_child(fragment), 4), 17, columns, index, ($$anchor, column) => {
		var label = root$1();
		...
		reset(label);
		template_effect(() => {
			set_attribute(input_1, "data-column-toggle", get(column).id);
...
//#endregion
//#region src/lib/components/context-menus/ContextMenu.svelte
var root = /* @__PURE__ */ from_html(`<hr class="context-divider"/>`);
var root_1 = /* @__PURE__ */ from_html(`<div class="context-item" style="opacity:0.5; cursor:default;">No tags</div>`);
var root_2 = /* @__PURE__ */ from_html(`<button class="context-item" role="menuitem"></button>`);
var root_3 = /* @__PURE__ */ from_html(`<div class="context-item has-submenu"><span> </span> <span style="opacity:0.5; font-size:10px;">►</span> <div class="context-submenu" style="position:absolute; left:100%; top:-6px; background:var(--bg-secondary); border:1px solid var(--border-color); border-radius:var(--radius-md); padding:6px 0; min-width:160px; box-shadow:0 8px 24px var(--shadow-color); display:none; z-index:1001;"><!></div></div>`);
function ContextMenu($$anchor, $$props) {
	push($$props, true);
	let tags = /* @__PURE__ */ state(proxy([]));
	onMount(() => {
		if (window.state && window.state.tags) set(tags, window.state.tags, true);
		const handleTagsUpdated = () => {
			if (window.state && window.state.tags) set(tags, window.state.tags, true);
		};
		document.addEventListener("simplefile:tags-updated", handleTagsUpdated);
		return () => {
			document.removeEventListener("simplefile:tags-updated", handleTagsUpdated);
		};
	});
	let entries = /* @__PURE__ */ user_derived(() => {
		return [
			{
				kind: "item",
				id: "ctx-open",
			...
			},
			{
				...
				id: "ctx-tags",
				label: "Tags",
				children: get(tags).map((t) => ({
					kind: "item",
					id: `ctx-tag-${t.id}`,
					label: `<span style="display:inline-block; width:12px; height:12px; border-radius:50%; background-color:${t.color}; margin-right:8px; vertical-align:middle;"></span>${t.name}`
				}))
			},
			{
				kind: "item",
			...
			},
			{
				kind: "item",
				id: "ctx-pack",
				label: "Pack into Folder..."
			},
			{
				kind: "item",
				id: "ctx-unpack",
				label: "Unpack Folder Here",
				hidden: true
			},
			{ kind: "divider" },
			{
	...
	each(first_child(fragment), 17, () => get(entries), index, ($$anchor, entry) => {
		...
		var alternate_3 = ($$anchor) => {
			...
			var consequent_3 = ($$anchor) => {
				...
				var alternate_1 = ($$anchor) => {
					...
					each(first_child(fragment_3), 17, () => get(entry).children, index, ($$anchor, child) => {
						...
						var alternate = ($$anchor) => {
							var button = root_2();
							html(button, () => get(child).label, true);
							reset(button);
							template_effect(() => set_attribute(button, "id", get(child).id));
							append($$anchor, button);
						};
						if_block(node_5, ($$render) => {
							if (get(child).kind === "divider") $$render(consequent_2);
				...
				if_block(node_3, ($$render) => {
					if (get(entry).children.length === 0) $$render(consequent_1);
					else $$render(alternate_1, -1);
				});
				reset(div_1);
				reset(div);
				template_effect(() => {
					styles = set_style(div, "position:relative; display:flex; justify-content:space-between; align-items:center;", styles, { display: get(entry).hidden ? "none" : void 0 });
					set_text(text, get(entry).label);
				});
				append($$anchor, div);
			};
			var alternate_2 = ($$anchor) => {
				var button_1 = root_2();
				let styles_1;
				html(button_1, () => get(entry).label, true);
				reset(button_1);
				template_effect(() => {
					set_attribute(button_1, "id", get(entry).id);
					styles_1 = set_style(button_1, "", styles_1, { display: get(entry).hidden ? "none" : void 0 });
				});
				append($$anchor, button_1);
			};
			if_block(node_2, ($$render) => {
				if (get(entry).children) $$render(consequent_3);
				else $$render(alternate_2, -1);
			});
			append($$anchor, fragment_2);
		};
		if_block(node_1, ($$render) => {
			if (get(entry).kind === "divider") $$render(consequent);
...
//#endregion
//#region src/lib/components/context-menus.ts
var mountedContextMenus = /* @__PURE__ */ new WeakMap();
var mountedColumnHeaderMenus = /* @__PURE__ */ new WeakMap();
...
function clearContextMenu(target) {
...
function renderContextMenu(target) {
	...
	target.dataset.svelteContextMenu = "true";
	mountedContextMenus.set(target, component);
}
function clearColumnHeaderMenu(target) {
	clearMounted(target, mountedColumnHeaderMenus, "svelteColumnHeaderMenu");
}
function renderColumnHeaderMenu(target, props = {}) {
	if (!target) return;
	...
	target.dataset.svelteColumnHeaderMenu = "true";
	mountedColumnHeaderMenus.set(target, component);
}
//#endregion
export { clearColumnHeaderMenu, clearContextMenu, renderColumnHeaderMenu, renderContextMenu };
