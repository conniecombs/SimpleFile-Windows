...
//#endregion
//#region src/lib/components/path-autocomplete/PathAutocomplete.svelte
var root = /* @__PURE__ */ from_html(`<div class="autocomplete-item" aria-selected="false" role="option" tabindex="-1"><span class="autocomplete-icon" aria-hidden="true">&#128193;</span> <span class="autocomplete-name"> </span></div>`);
var root_1 = /* @__PURE__ */ from_html(`<div class="path-autocomplete-items"></div>`);
function PathAutocomplete($$anchor, $$props) {
	push($$props, true);
	let items = prop($$props, "items", 19, () => []), onChoose = prop($$props, "onChoose", 3, void 0), onDismiss = prop($$props, "onDismiss", 3, void 0), separator = prop($$props, "separator", 3, "/");
	...
	function choose(path, event) {
		event?.preventDefault();
		event?.stopPropagation();
		onChoose()?.(withTrailingSeparator(path));
	}
	function optionElements() {
		return Array.from(get(rootElement)?.querySelectorAll(".autocomplete-item") ?? []);
	}
	function focusByOffset(offset) {
		const options = optionElements();
		if (options.length === 0) return;
		const currentIndex = options.findIndex((option) => option === document.activeElement);
		options[currentIndex === -1 ? offset > 0 ? 0 : options.length - 1 : (currentIndex + offset + options.length) % options.length]?.focus();
	}
	...
	function activeOption() {
		...
		return active instanceof HTMLElement ? active.closest(".autocomplete-item") : null;
	}
	function handleKeydown(event) {
		...
		if (event.key === "Enter" || event.key === "Tab") {
			const path = activeOption()?.dataset.path;
			if (!path) return;
			choose(path, event);
			return;
		}
	...
	var div = root_1();
	each(div, 21, items, (item) => item.path, ($$anchor, item) => {
		var div_1 = root();
...
delegate(["keydown", "mousedown"]);
//#endregion
//#region src/lib/components/path-autocomplete.ts
var mountedDropdowns = /* @__PURE__ */ new WeakMap();
function clearPathAutocomplete(target) {
	if (!target) return;
...
function renderPathAutocomplete(target, props = {}) {
	if (!target) return;
...
//#endregion
export { clearPathAutocomplete, renderPathAutocomplete };
