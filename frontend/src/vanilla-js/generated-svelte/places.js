...
//#endregion
//#region src/lib/components/places/BookmarkList.svelte
var root$4 = /* @__PURE__ */ from_html(`<div class="top-menu-empty">No bookmarks yet</div>`);
var root_1$2 = /* @__PURE__ */ from_html(`<div class="bookmark-item" role="listitem"><span class="bookmark-icon" aria-hidden="true">&#128193;</span> <span class="bookmark-name"> </span> <button type="button" class="bookmark-remove" title="Remove bookmark" aria-label="Remove bookmark">&times;</button></div>`);
function BookmarkList($$anchor, $$props) {
	push($$props, true);
	let bookmarks = prop($$props, "bookmarks", 19, () => []);
	...
	var alternate = ($$anchor) => {
		...
		each(first_child(fragment_1), 17, bookmarks, (bookmark) => bookmark.id, ($$anchor, bookmark) => {
			var div_1 = root_1$2();
			...
			reset(div_1);
			template_effect(() => {
				set_attribute(div_1, "data-path", get(bookmark).path);
				set_attribute(div_1, "data-id", get(bookmark).id);
				set_text(text, get(bookmark).name);
				set_attribute(button, "data-id", get(bookmark).id);
				set_attribute(button, "data-path", get(bookmark).path);
			});
			delegated("click", div_1, () => $$props.onNavigate?.(get(bookmark).path));
			delegated("click", button, (e) => {
				e.stopPropagation();
				$$props.onRemove?.(get(bookmark).id);
			});
			append($$anchor, div_1);
		});
	...
	if_block(node, ($$render) => {
		if (bookmarks().length === 0) $$render(consequent);
...
function QuickAccessList($$anchor, $$props) {
	...
	var fragment = comment();
	each(first_child(fragment), 19, locations, (location, index) => `${location.name}-${location.path || location.action || index}`, ($$anchor, location) => {
		var div = root$2();
		var span = child(div);
		var text = child(span, true);
		reset(span);
		...
		reset(div);
		template_effect(() => {
			set_attribute(div, "data-path", get(location).path || void 0);
			set_attribute(div, "data-action", get(location).action || void 0);
			set_text(text, get(location).icon);
			set_text(text_1, get(location).name);
		});
		append($$anchor, div);
	});
	append($$anchor, fragment);
...
//#endregion
//#region src/lib/components/places/RecentLocationsList.svelte
var root$1 = /* @__PURE__ */ from_html(`<div class="top-menu-empty">No recent items</div>`);
var root_1$1 = /* @__PURE__ */ from_html(`<div class="recent-item" role="listitem"><span class="recent-icon" aria-hidden="true">&#128193;</span> <span class="recent-name"> </span></div>`);
function RecentLocationsList($$anchor, $$props) {
	push($$props, true);
	let recentLocations = prop($$props, "recentLocations", 19, () => []);
	var fragment = comment();
	var node = first_child(fragment);
	var consequent = ($$anchor) => {
	...
	var alternate = ($$anchor) => {
		var fragment_1 = comment();
		each(first_child(fragment_1), 17, recentLocations, (recent) => recent.path, ($$anchor, recent) => {
			var div_1 = root_1$1();
			var span = sibling(child(div_1), 2);
			var text = child(span, true);
			reset(span);
			reset(div_1);
			template_effect(() => {
				set_attribute(div_1, "data-path", get(recent).path);
				set_attribute(span, "title", get(recent).path);
				set_text(text, get(recent).name);
			});
			append($$anchor, div_1);
		});
		append($$anchor, fragment_1);
	...
	if_block(node, ($$render) => {
		if (recentLocations().length === 0) $$render(consequent);
		else $$render(alternate, -1);
	});
	append($$anchor, fragment);
	pop();
...
//#endregion
//#region src/lib/components/places/SmartFoldersList.svelte
var root = /* @__PURE__ */ from_html(`<div class="top-menu-empty">No smart folders yet</div>`);
var root_1 = /* @__PURE__ */ from_html(`<div class="bookmark-item" role="listitem"><span class="bookmark-icon" aria-hidden="true"> </span> <span class="bookmark-name"> </span> <button type="button" class="bookmark-remove" title="Remove smart folder" aria-label="Remove smart folder">&times;</button></div>`);
function SmartFoldersList($$anchor, $$props) {
	push($$props, true);
	let smartFolders = prop($$props, "smartFolders", 19, () => []);
	var fragment = comment();
	var node = first_child(fragment);
	var consequent = ($$anchor) => {
		append($$anchor, root());
	...
	var alternate = ($$anchor) => {
		var fragment_1 = comment();
		each(first_child(fragment_1), 17, smartFolders, (folder) => folder.id, ($$anchor, folder) => {
			var div_1 = root_1();
			...
			reset(div_1);
			template_effect(() => {
				set_attribute(div_1, "data-id", get(folder).id);
				set_text(text, get(folder).icon || "🔍");
				set_text(text_1, get(folder).name);
				set_attribute(button, "data-id", get(folder).id);
			});
			delegated("click", div_1, () => $$props.onNavigate?.(get(folder)));
			delegated("click", button, (e) => {
				e.stopPropagation();
				$$props.onRemove?.(get(folder).id);
			});
			append($$anchor, div_1);
		});
		append($$anchor, fragment_1);
	...
	if_block(node, ($$render) => {
		if (smartFolders().length === 0) $$render(consequent);
		else $$render(alternate, -1);
	});
	append($$anchor, fragment);
	pop();
...
delegate(["click"]);
//#endregion
//#region src/lib/components/places.ts
var mountedLists = /* @__PURE__ */ new WeakMap();
function clearMountedList(target) {
	if (!target) return;
...
function renderRecentLocationsList(target, props = {}) {
	if (!target) return;
	clearMountedList(target);
	const component = mount(RecentLocationsList, {
		target,
		props: { recentLocations: props.recentLocations ?? [] }
	});
	mountedLists.set(target, component);
	target.querySelectorAll(".recent-item").forEach((item) => {
...
function renderQuickAccessList(target, props = {}) {
	if (!target) return;
	clearMountedList(target);
	...
	mountedLists.set(target, component);
	target.querySelectorAll(".quick-access-item").forEach((item) => {
		item.addEventListener("click", () => {
			const action = item.dataset.action;
			const path = item.dataset.path;
			if (action) props.onAction?.(action);
			else if (path) props.onNavigate?.(path);
		});
	});
}
