...
//#endregion
//#region src/lib/components/search-chrome/QuickFilterBar.svelte
var root$1 = /* @__PURE__ */ from_html(`<span class="quick-filter-icon" aria-hidden="true">&#128270;</span> <input class="quick-filter-input" id="filter-input" placeholder="Filter files... (Escape to clear)" aria-label="Filter current directory" type="text"/> <span class="quick-filter-count" id="filter-count"> </span> <button class="quick-filter-clear" id="filter-clear" title="Clear filter (Escape)" aria-label="Clear filter" type="button">&times;</button>`, 1);
function QuickFilterBar($$anchor, $$props) {
	push($$props, true);
	let initialCount = prop($$props, "initialCount", 3, ""), initialQuery = prop($$props, "initialQuery", 3, ""), onClear = prop($$props, "onClear", 3, void 0), onInput = prop($$props, "onInput", 3, void 0);
	let inputElement = /* @__PURE__ */ state(void 0);
	let countText = /* @__PURE__ */ state("");
	...
	user_effect(() => {
		set(countText, initialCount());
		set(query, initialQuery());
	});
	function setCount(nextCount) {
		set(countText, nextCount, true);
	}
	function setQuery(nextQuery) {
		set(query, nextQuery, true);
	}
	function focusInput() {
		get(inputElement)?.focus();
	}
	function emitQuickFilterEvent(type, event, detail = {}) {
	...
	function handleInput(event) {
		emitQuickFilterEvent(QUICK_FILTER_INPUT_EVENT, event, { query: get(query) });
		onInput()?.(get(query));
	}
	...
	template_effect(() => set_text(text, get(countText)));
...
//#endregion
//#region src/lib/components/search-chrome/SearchResultsHeader.svelte
var root = /* @__PURE__ */ from_html(`<span class="search-results-count"> </span> <button class="btn btn-secondary btn-sm search-save-btn" type="button"> </button> <button class="btn btn-secondary btn-sm search-clear-btn" type="button"> </button>`, 1);
function SearchResultsHeader($$anchor, $$props) {
	push($$props, true);
	let clearLabel = prop($$props, "clearLabel", 3, "Clear"), saveLabel = prop($$props, "saveLabel", 3, "Save Search"), label = prop($$props, "label", 3, ""), onClear = prop($$props, "onClear", 3, void 0), onSave = prop($$props, "onSave", 3, void 0);
	...
	function handleClear(event) {
		event.currentTarget?.dispatchEvent(new CustomEvent(SEARCH_RESULTS_CLEAR_EVENT, { bubbles: true }));
		onClear()?.();
	}
	function handleSave(event) {
		event.currentTarget?.dispatchEvent(new CustomEvent(SEARCH_RESULTS_SAVE_EVENT, { bubbles: true }));
		onSave()?.();
	}
	var fragment = root();
	...
	delegated("click", button, handleSave);
	delegated("click", button_1, handleClear);
	append($$anchor, fragment);
	pop();
}
delegate(["click"]);
//#endregion
//#region src/lib/components/search-chrome.ts
var mountedQuickFilters = /* @__PURE__ */ new WeakMap();
var mountedSearchHeaders = /* @__PURE__ */ new WeakMap();
function clearQuickFilterBar(target) {
	if (!target) return;
...
function renderQuickFilterBar(target, props = {}) {
	if (!target) return;
...
function updateQuickFilterCount(target, countText) {
...
function focusQuickFilterInput(target) {
	if (!target) return;
	mountedQuickFilters.get(target)?.focusInput?.();
}
function clearSearchResultsHeader(target) {
	if (!target) return;
...
function renderSearchResultsHeader(parent, before, props) {
...
//#endregion
export { clearQuickFilterBar, clearSearchResultsHeader, focusQuickFilterInput, renderQuickFilterBar, renderSearchResultsHeader, updateQuickFilterCount };
