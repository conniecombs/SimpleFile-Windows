...
//#endregion
//#region src/lib/components/status-bar/StatusBar.svelte
var root = /* @__PURE__ */ from_html(`<span class="status-bar-section status-selected"> </span>`);
var root_1 = /* @__PURE__ */ from_html(`<span class="status-bar-section status-path"> </span>`);
var root_2 = /* @__PURE__ */ from_html(`<span class="status-bar-section status-git"><span class="status-git-label">Git</span> <span class="status-git-branch"> </span> <span> </span></span>`);
var root_3 = /* @__PURE__ */ from_html(`<span class="status-bar-section status-disk"><span class="status-disk-text"> </span> <span class="status-disk-meter" role="meter" aria-label="Disk usage" aria-valuemin="0" aria-valuemax="100"><span></span></span></span>`);
var root_4 = /* @__PURE__ */ from_html(`<span class="status-bar-section status-items"> </span> <!> <!> <!> <!>`, 1);
function StatusBar($$anchor, $$props) {
	push($$props, true);
	let currentPath = prop($$props, "currentPath", 3, null), disk = prop($$props, "disk", 3, null), git = prop($$props, "git", 3, null), selectedCount = prop($$props, "selectedCount", 3, 0), selectedSizeText = prop($$props, "selectedSizeText", 3, null), totalItems = prop($$props, "totalItems", 3, 0);
	var fragment = root_4();
	...
	var consequent = ($$anchor) => {
		...
		reset(span_1);
		template_effect(() => set_text(text_1, `${selectedCount() ?? ""} selected${selectedSizeText() ? ` (${selectedSizeText()})` : ""}`));
		append($$anchor, span_1);
	};
	if_block(node, ($$render) => {
		if (selectedCount() > 0) $$render(consequent);
	});
	...
	var consequent_1 = ($$anchor) => {
		...
		reset(span_2);
		template_effect(() => {
			set_attribute(span_2, "title", currentPath());
			set_text(text_2, currentPath());
		});
		append($$anchor, span_2);
	};
	if_block(node_1, ($$render) => {
		if (currentPath()) $$render(consequent_1);
	});
	var node_2 = sibling(node_1, 2);
	var consequent_2 = ($$anchor) => {
		var span_3 = root_2();
		var span_4 = sibling(child(span_3), 2);
		...
		reset(span_3);
		template_effect(() => {
			set_attribute(span_3, "title", git().title);
			set_text(text_3, git().branch);
			set_class(span_5, 1, `status-git-state ${git().changeCount > 0 ? "status-git-state--dirty" : "status-git-state--clean"}`);
			set_text(text_4, git().changeText);
		});
		append($$anchor, span_3);
	};
	if_block(node_2, ($$render) => {
		if (git()) $$render(consequent_2);
	});
	...
	var consequent_3 = ($$anchor) => {
		...
		reset(span_6);
		template_effect(($0) => {
			set_attribute(span_6, "title", `${disk().name}: ${disk().freeText}`);
			set_text(text_5, disk().freeText);
			set_attribute(span_8, "aria-valuenow", $0);
			set_class(span_9, 1, `status-disk-fill ${disk().usedPercent >= 85 ? "status-disk-fill--warning" : ""}`);
			styles = set_style(span_9, "", styles, { width: `${disk().usedPercent}%` });
		}, [() => Math.round(disk().usedPercent)]);
		append($$anchor, span_6);
	};
	if_block(node_3, ($$render) => {
		if (disk()) $$render(consequent_3);
	});
	template_effect(() => set_text(text, `${totalItems() ?? ""} item${totalItems() === 1 ? "" : "s"}`));
	append($$anchor, fragment);
	pop();
}
//#endregion
//#region src/lib/components/status-bar.ts
var mountedStatusBars = /* @__PURE__ */ new WeakMap();
function clearStatusBar(target) {
	if (!target) return;
...
function renderStatusBar(target, props) {
	if (!target) return;
...
//#endregion
export { clearStatusBar, renderStatusBar };
