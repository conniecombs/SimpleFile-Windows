...
//#endregion
//#region src/lib/components/tabs/TabsBar.svelte
var root = /* @__PURE__ */ from_html(`<button type="button" class="tab-new-btn" title="New Tab" aria-label="Open new tab">+</button>`);
var root_1 = /* @__PURE__ */ from_html(`<div role="tab"><span class="tab-icon" aria-hidden="true">&#128193;</span> <span class="tab-title"> </span> <button type="button" class="tab-close" title="Close Tab" aria-label="Close tab">&times;</button></div> <!>`, 1);
var root_2 = /* @__PURE__ */ from_html(`<!> <!>`, 1);
function TabsBar($$anchor, $$props) {
	push($$props, true);
	let activeTabId = prop($$props, "activeTabId", 3, null), tabs = prop($$props, "tabs", 19, () => []);
	...
	function emitTabEvent(type, event, detail = {}) {
	...
	function handleTabKeydown(event, tab) {
		switch (event.key) {
			...
			case "ArrowLeft":
				event.preventDefault();
				emitTabEvent(TAB_FOCUS_MOVE_EVENT, event, {
					direction: -1,
					tabId: tab.id
				});
				break;
	...
	function handleCloseClick(event, tab) {
		event.preventDefault();
	...
	each(node, 17, tabs, (tab) => tab.id, ($$anchor, tab) => {
		...
		var consequent = ($$anchor) => {
			var button_1 = root();
			delegated("click", button_1, handleNewClick);
			append($$anchor, button_1);
		};
		var d = /* @__PURE__ */ user_derived(() => isActive(get(tab)));
		if_block(node_1, ($$render) => {
			if (get(d)) $$render(consequent);
		});
		template_effect(($0, $1, $2) => {
			set_class(div, 1, $0);
			set_attribute(div, "data-tab-id", get(tab).id);
			set_attribute(div, "aria-selected", $1);
			set_attribute(div, "tabindex", $2);
			set_attribute(span, "title", get(tab).path);
			set_text(text, get(tab).title);
			set_attribute(button, "data-tab-id", get(tab).id);
		}, [
			() => `tab${isActive(get(tab)) ? " active" : ""}`,
			() => isActive(get(tab)),
			() => isActive(get(tab)) ? 0 : -1
		]);
		delegated("click", div, (event) => handleTabClick(event, get(tab)));
		event("auxclick", div, (event) => handleTabAuxClick(event, get(tab)));
		delegated("keydown", div, (event) => handleTabKeydown(event, get(tab)));
		delegated("click", button, (event) => handleCloseClick(event, get(tab)));
		append($$anchor, fragment_1);
	});
	var node_2 = sibling(node, 2);
	var consequent_1 = ($$anchor) => {
		var button_2 = root();
		delegated("click", button_2, handleNewClick);
		append($$anchor, button_2);
	};
	var d_1 = /* @__PURE__ */ user_derived(() => !hasActiveTab());
...
//#endregion
//#region src/lib/components/tabs.ts
var mountedTabs = /* @__PURE__ */ new WeakMap();
function clearTabsBar(target) {
	if (!target) return;
...
function renderTabsBar(target, props) {
	if (!target) return;
	clearTabsBar(target);
...
//#endregion
export { clearTabsBar, renderTabsBar };
