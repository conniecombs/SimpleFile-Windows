...
//#endregion
//#region src/lib/components/toasts/toastStore.ts
var FADE_OUT_MS = 300;
var nextToastId = 1;
...
function dismissToast(id, animated = true) {
	clearToastTimers(id);
	if (!animated) {
		removeToast(id);
		return;
	...
	toasts.update((items) => items.map((toast) => {
	...
	if (shouldScheduleRemoval) timers.set(id, [window.setTimeout(() => removeToast(id), FADE_OUT_MS)]);
}
function addToast(options) {
	...
	if (toast.durationMs > 0) timers.set(id, [window.setTimeout(() => dismissToast(id), toast.durationMs)]);
	return id;
}
function triggerToastAction(toast) {
	try {
...
//#endregion
//#region src/lib/components/toasts/ToastHost.svelte
var root = /* @__PURE__ */ from_html(`<button type="button" class="toast-undo btn btn-link"> </button>`);
var root_1 = /* @__PURE__ */ from_html(`<div><span> </span> <!></div>`);
var root_2 = /* @__PURE__ */ from_html(`<div class="svelte-toast-region" aria-live="polite" aria-atomic="true"></div>`);
function ToastHost($$anchor, $$props) {
	push($$props, false);
	const $toasts = () => store_get(toasts, "$toasts", $$stores);
	const [$$stores, $$cleanup] = setup_stores();
	init();
	var div = root_2();
	each(div, 5, $toasts, (toast) => toast.id, ($$anchor, toast) => {
		var div_1 = root_1();
		let classes;
		...
		reset(div_1);
		template_effect(() => {
			classes = set_class(div_1, 1, `toast ${get(toast).kind}`, null, classes, { "fade-out": get(toast).fading });
			set_attribute(div_1, "role", get(toast).kind === "error" ? "alert" : "status");
...
delegate(["click"]);
//#endregion
//#region src/lib/components/toasts.ts
var TOAST_ROOT_ID = "simplefile-svelte-toast-root";
var mounted = false;
...
function showToast(message, kind, durationMs) {
	ensureToastHost();
	return addToast({
		message: normalizeMessage(message, kind === "error" ? "An error occurred" : "Operation completed"),
		kind,
		durationMs
	});
}
function showSuccess(message) {
	return showToast(message, "success", 2e3);
}
function showError(message) {
	return showToast(message, "error", 3e3);
}
function showUndoableSuccess(message, onUndo) {
	ensureToastHost();
	return addToast({
...
//#endregion
export { showError, showSuccess, showToast, showUndoableSuccess };
