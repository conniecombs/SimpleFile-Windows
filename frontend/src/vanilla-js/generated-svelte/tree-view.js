...
//#endregion
//#region src/lib/components/tree-view/TreeView.svelte
var treeSkeleton = ($$anchor, depth = noop) => {
	var div = root_2();
	each(div, 20, () => [
		0,
		1,
		2
	], (index) => index, ($$anchor, index) => {
		var div_1 = root_1();
		template_effect(() => set_attribute(div_1, "data-depth", depth() + 1));
		append($$anchor, div_1);
	});
	reset(div);
	append($$anchor, div);
};
var root_1 = /* @__PURE__ */ from_html(`<div class="skeleton-tree-item"><div class="skeleton skeleton-expand"></div> <div class="skeleton skeleton-folder"></div> <div class="skeleton skeleton-tree-name"></div></div>`);
...
function TreeView($$anchor, $$props) {
	...
	const treeNode = ($$anchor, node = noop, depth = noop) => {
		...
		var consequent = ($$anchor) => {
			...
			template_effect(($0) => {
				...
				set_attribute(button, "data-path", node().path);
				set_attribute(button, "aria-label", node().isExpanded ? `Collapse ${node().name}` : `Expand ${node().name}`);
			}, [() => clsx(expandClass(node()))]);
			delegated("click", button, (event) => handleExpandClick(event, node()));
			append($$anchor, button);
		};
		var alternate = ($$anchor) => {
			var span = root_4();
			template_effect(() => set_attribute(span, "data-path", node().path));
			append($$anchor, span);
		};
		if_block(node_1, ($$render) => {
			if (node().hasChildren) $$render(consequent);
		...
		var consequent_4 = ($$anchor) => {
			...
			var consequent_2 = ($$anchor) => {
				treeSkeleton($$anchor, depth);
			};
			var consequent_3 = ($$anchor) => {
				var div_4 = root_6();
				each(div_4, 21, () => node().children, (child) => child.path, ($$anchor, child) => {
					treeNode($$anchor, () => get(child), () => depth() + 1);
				});
				reset(div_4);
				template_effect(() => set_class(div_4, 1, `tree-children${node().isExpanded ? "" : " collapsed"}`));
				append($$anchor, div_4);
			};
			if_block(node_4, ($$render) => {
				if (node().isExpanded && !node().isLoaded) $$render(consequent_2);
				else if (node().children.length > 0) $$render(consequent_3, 1);
			});
			append($$anchor, fragment);
		};
		if_block(node_3, ($$render) => {
			if (node().hasChildren) $$render(consequent_4);
		});
		reset(div_2);
		template_effect(($0) => {
			set_attribute(div_2, "data-path", node().path);
			set_class(div_3, 1, $0);
			set_attribute(div_3, "data-path", node().path);
			set_attribute(div_3, "data-depth", depth());
			set_attribute(div_3, "aria-expanded", node().isExpanded);
			set_attribute(div_3, "aria-selected", node().isActive);
			set_text(text, node().icon);
			set_text(text_1, node().name);
		}, [() => clsx(itemClass(node()))]);
		delegated("click", div_3, (event) => handleItemClick(event, node()));
		delegated("keydown", div_3, (event) => handleItemKeydown(event, node()));
		append($$anchor, div_2);
	};
	let roots = prop($$props, "roots", 19, () => []);
	...
	function handleExpandClick(event, node) {
		event.preventDefault();
		event.stopPropagation();
		emitTreeEvent(TREE_NODE_TOGGLE_EVENT, event, node);
	}
	function handleItemClick(event, node) {
		emitTreeEvent(TREE_NODE_OPEN_EVENT, event, node);
	}
	function handleItemKeydown(event, node) {
		switch (event.key) {
			case "ArrowDown":
				event.preventDefault();
				event.stopPropagation();
			...
			case "ArrowLeft":
				event.preventDefault();
				event.stopPropagation();
				if (node.hasChildren && node.isExpanded) emitTreeEvent(TREE_NODE_TOGGLE_EVENT, event, node);
				else emitTreeEvent(TREE_NODE_FOCUS_PARENT_EVENT, event, node);
				break;
			case "Enter":
			case " ":
				event.preventDefault();
	...
	each(first_child(fragment_3), 17, roots, (root) => root.path, ($$anchor, root) => {
		treeNode($$anchor, () => get(root), () => 0);
	});
...
//#endregion
//#region src/lib/components/tree-view.ts
var mountedTreeViews = /* @__PURE__ */ new WeakMap();
function clearTreeView(target) {
	if (!target) return;
...
function renderTreeView(target, props = {}) {
	if (!target) return;
	clearTreeView(target);
	const component = mount(TreeView, {
		target,
		props: { roots: props.roots ?? [] }
	});
	mountedTreeViews.set(target, component);
}
//#endregion
export { clearTreeView, renderTreeView };
