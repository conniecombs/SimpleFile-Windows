...
$msi = Get-ChildItem -Path $bundleDir -Filter "SimpleFile_*_x64-winui.msi" -File |
...
$smokeRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("simplefile-winui-msi-smoke-" + [System.Guid]::NewGuid().ToString("N"))
...
try {
    ...
    $exe = Get-ChildItem -Path $extractDir -Filter "SimpleFile.exe" -Recurse -File |
        Select-Object -First 1
    if (-not $exe) {
        throw "WinUI MSI extraction did not contain SimpleFile.exe under $extractDir."
    }
