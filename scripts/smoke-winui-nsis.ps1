...
$installer = Get-ChildItem -Path $bundleDir -Filter "SimpleFile_*_x64-winui-setup.exe" -File |
...
function Get-WinUIInstall {
    Get-ItemProperty "HKCU:\Software\Microsoft\Windows\CurrentVersion\Uninstall\*" -ErrorAction SilentlyContinue |
        Where-Object { $_.DisplayName -eq "SimpleFile (WinUI)" } |
        Select-Object -First 1
}
...
function Find-WinUIExecutable($installed) {
    ...
    $candidates += Join-Path $env:LOCALAPPDATA "Programs\SimpleFile-WinUI\SimpleFile.exe"
...
function Invoke-Uninstall($installed) {
    ...
    if (-not $uninstallerPath -or -not (Test-Path -LiteralPath $uninstallerPath)) {
        throw "Could not find SimpleFile (WinUI) uninstaller. UninstallString: '$uninstallCommand'."
    }
...
if ($existing) {
    throw "SimpleFile (WinUI) is already installed at '$($existing.InstallLocation)'. Uninstall it before running this smoke test."
}
...
try {
    ...
    if (-not $exePath) {
        throw "Installed SimpleFile.exe was not found."
    }

    $servicePath = Join-Path (Split-Path -Parent $exePath) "simplefile-service.exe"
    if (-not (Test-Path -LiteralPath $servicePath)) {
        throw "Installed simplefile-service.exe was not found next to SimpleFile.exe."
    }

    Write-Host "Installed SimpleFile (WinUI) $($installed.DisplayVersion) at $exePath."
    ...
    if (-not $KeepInstalled) {
        $installed = Get-WinUIInstall
        if ($installed) {
            Write-Host "Uninstalling SimpleFile (WinUI) $($installed.DisplayVersion)."
            Invoke-Uninstall $installed
            Write-Host "Uninstalled SimpleFile (WinUI)."
        }
    } else {
        Write-Host "Keeping installed SimpleFile (WinUI) because -KeepInstalled was supplied."
    }
}
