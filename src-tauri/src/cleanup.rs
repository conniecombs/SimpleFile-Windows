use crate::models::{
    CleanupResult, DuplicateCheckFile, DuplicateCheckGroup, DuplicateCheckResult, DuplicateGroup,
    ProgressUpdate,
};
...
const DUPLICATE_CHECK_OPERATION_ID: &str = "duplicate_check";
const FIRST_PASS_PROGRESS_INTERVAL: u64 = 128;
const HASH_PROGRESS_INTERVAL: u64 = 16;
const DEFAULT_DUPLICATE_MIN_SIZE: u64 = 1;
const DEFAULT_PARTIAL_HASH_BYTES: u64 = 1024 * 1024;
const MAX_PARTIAL_HASH_BYTES: u64 = 16 * 1024 * 1024;
...
pub async fn duplicate_check(
    ...
    let cancel = state.duplicate_check_cancel.clone();
    cancel.store(false, Ordering::Relaxed);

    let result = match tokio::task::spawn_blocking(move || {
        scan_duplicate_check(&root, options, &cancel, |current, total, item| {
            emit_duplicate_progress(&app, current, total, item);
        })
    })
    .await
    {
        Ok(result) => result,
        Err(e) => Err(format!("Duplicate checker task failed: {e}")),
    };

    state.duplicate_check_cancel.store(false, Ordering::Relaxed);
    result
}

#[tauri::command]
pub fn cancel_duplicate_check(state: tauri::State<'_, Arc<AppState>>) {
    state.cancel_duplicate_check();
}
...
fn emit_duplicate_progress(app: &AppHandle, current: u64, total: u64, current_item: &str) {
    let _ = app.emit(
        "operation-progress",
        ProgressUpdate {
            operation_id: DUPLICATE_CHECK_OPERATION_ID.to_string(),
    ...
    F: FnMut(u64, u64, &str),
{
    ...
    let mut groups: Vec<DuplicateCheckGroup> = full_hash_map
        ...
        .filter_map(|((size, hash), mut files)| {
            ...
            let id = duplicate_group_id(size, &hash);
